                            ...
                            GestureDetector(
                              onTap: () async {
                                _key.currentState.showBottomSheet((context) =>
                                    CatButtonFilters("From Search Area",
                                        widget.name, catName));
                              },
                              child: Image.asset(
...
//                                                fontWeight: FontWeight.bold,
  ...
  Future getProductList(String key) async {
    ApiRepository.loadHomeSearch(key, currentProductCount)
  ...
  callPrice() async {
    ...
    for (int i = 0; i < productList.items.length; i++) {
      if (productList.items[i].typeId == "configurable") {
        ...
        for (var j = 0; j < productList.items[i].customAttributes.length; j++) {
          if (productList.items[i].customAttributes[j].attributeCode ==
            ...
            setState(() {
              ...
              productPrice.insert(i,
                  double.parse(productList.items[i].customAttributes[j].value));
            });
          }
        }
        if (inside == true) {
          setState(() {
            print("inside spe1c");
            productPrice.insert(i, productList.items[i].price);
          });
        }
      }
    }
...
//
  ...
  Future getSortbyPosition() async {
    ...
    ApiRepository.sortOnPostionForName(widget.name, currentProductCount)
  ...
  Future getSortbyName() async {
    ...
    ApiRepository.sortOnNameForName(widget.name, currentProductCount)
        .then((userDataFromServer) async {
      if (userDataFromServer != null) {
        setState(() {
          _progressController = false;
  ...
  Future getSortbyPrice() async {
    ...
    ApiRepository.sortOnPriceForName(widget.name, currentProductCount)
        .then((userDataFromServer) async {
      if (userDataFromServer != null) {
        setState(() {
          _progressController = false;
